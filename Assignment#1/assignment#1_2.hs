...
-- determine which color to fill
  -- if r < 0.1 then fill the region with red
  | r < 0.1 = (rs, dup ++ " fill=\"rgb(" ++ (show 255) ++ "," ++
                                               (show 0) ++ "," ++
                                               (show 0) ++ ")\" />\n")
  -- Else if r < 0.2 then fill the region with dark organge
  | r < 0.2 = (rs, dup ++ " fill=\"rgb(" ++ (show 255) ++ "," ++
                                               (show 140) ++ "," ++
                                               (show 0) ++ ")\" />\n")
  -- Else if r < 0.3 then fill the region with yellow
  | r < 0.3   = (rs, dup ++ " fill=\"rgb(" ++ (show 255) ++ "," ++
                                               (show 255) ++ "," ++
                                               (show 0) ++ ")\" />\n")
  -- Else if r < 0.4 then fill the region with Lime Green
  | r < 0.4   = (rs, dup ++ " fill=\"rgb(" ++ (show 50) ++ "," ++
                                              (show 205) ++ "," ++
                                              (show 50) ++ ")\" />\n")
  -- Else if r < 0.5 then fill the region with Lavender
  | r < 0.5   = (rs, dup ++ " fill=\"rgb(" ++ (show 230) ++ "," ++
                                              (show 230) ++ "," ++
                                              (show 250) ++ ")\" />\n")
  -- Else if r < 0.6 then fill the region with Deep Sky Blue
  | r < 0.6   = (rs, dup ++ " fill=\"rgb(" ++ (show 0) ++ "," ++
                                              (show 191) ++ "," ++
                                              (show 255) ++ ")\" />\n")
  -- Else if r < 0.7 then fill the region with purple
  | r < 0.7   = (rs, dup ++ " fill=\"rgb(" ++ (show 160) ++ "," ++
                                              (show 32) ++ "," ++
                                              (show 240) ++ ")\" />\n")
...
decision x y w h (one:two:three:four:five:six:seven:eight:rs) -- divide the list of random floating numbers into variables
  ...
  | w > (width `div` 5) && h > (height `div` 5) ||
    w > random_int_w_1 && h > random_int_w_1     =
  ...
  | w > (width `div` 3) ||  w > random_int_w_1   =
  ...
  | h > (height `div` 3) || h > random_int_w_1   =
  ...
  where
    new_w_1 = randomInt (round(fromIntegral w * 0.21)) (round(fromIntegral w * 0.67)) one
    new_h_1 = randomInt (round(fromIntegral h * 0.33)) (round(fromIntegral h * 0.27)) two
    new_w_2 = randomInt (round(fromIntegral w * 0.33)) (round(fromIntegral w * 0.47)) three
    new_h_3 = randomInt (round(fromIntegral h * 0.1)) (round(fromIntegral h * 0.67)) four
    random_int_w_1 = randomInt 70 (round(fromIntegral w * 1.2)) five
    random_int_h_1 = randomInt 70 (round(fromIntegral h * 1.5)) six
    random_int_w_2 = randomInt 70 (round(fromIntegral w * 1.5)) seven
    random_int_h_3 = randomInt 70 (round(fromIntegral h * 1.5)) eight
...
main = do
  ...
  writeFile "mondrian_2_other.html" (prefix ++ image ++ suffix)
